...
class CodePilotViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun onUserPromptSent(text: String) {
        ...
        val current = _uiState.value.currentResponseRun
    ...
    /** Called from the WebView bridge on the false -> true generation edge. Idempotent. */
    fun onAiGenerationStarted() {
        val state = _uiState.value
        if (isProcessingRun) return
        if (state.currentResponseRun?.status == "running") return

        val runNumber = state.responseRunsHistory.size + 1
        val activities = mutableListOf<AgentActivityItem>()
        pendingUserPrompt?.let { prompt ->
            activities.add(
                AgentActivityItem(
                    type = AgentActivityType.PROMPT_SENT,
                    title = "You: " + prompt,
