...
enum class AgentActivityType {
    ...
    VALIDATE,
    ARTIFACT_READY,
    /** Live preview of the in-progress AI Studio reply: parsed into prose + file-operation
     *  cards only (see [TimelineStreamSegment]) — never carries a visible label of its own. */
    STREAM_PREVIEW
}

/** Lifecycle state shown on a live file-operation card while a response streams in. */
enum class FileCardState {
    CREATING,
    WRITING,
    UPDATING,
    ANALYSING,
...
/**
 * One parsed piece of the live AI Studio stream, already classified into either explanatory
 * text or a code block. A [FileCard] never carries the underlying source text — only the
 * metadata needed to render a compact card — so raw generated code is never retained in UI
 * state or shown to the user.
 */
sealed class TimelineStreamSegment {
    data class Prose(val text: String) : TimelineStreamSegment()
    data class FileCard(
        val cardKey: String,
        val fileName: String?,
        val language: String,
        val lineCount: Int,
        val state: FileCardState
...
data class AgentActivityItem(
    ...
    val streamSegments: List<TimelineStreamSegment> = emptyList(),
...
class CodePilotViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun beginOrUpdateRun(prompt: String?, markWatching: Boolean) {
        synchronized(runStartLock) {
            ...
            // markWatching used to append a generic "Watching Google AI Studio..." label here.
            // That label (and its streaming-update counterpart) has been removed from the
            // Timeline entirely: the user already sees the response arriving via live text and
            // file cards, so no separate monitoring message is shown or substituted in its place.
     ...
     * Keeps a single live [AgentActivityType.STREAM_PREVIEW] item in the Plan timeline updated —
     * parsed into prose text and file-operation card metadata only. The raw text handed in here
     * (which can contain full, unfenced source lines straight from the AI Studio DOM) is used
     * only to compute that metadata; it is never itself written into UI state, so no generated
     * source ever reaches the Timeline as visible or stored raw text.
     */
    fun onAiStreamingUpdate(partial: String) {
        ...
        val existingPaths = state.files.flatMap { listOf(it.path, it.name) }.toSet()
        val segments = try {
            buildTimelineStreamSegments(text, existingPaths)
        } catch (e: Exception) {
            // Parsing is best-effort: on any failure, leave the previously rendered Timeline
            // content untouched rather than surfacing a raw error or stack trace.
            return
        }

        val existing = run.activities.indexOfFirst { it.type == AgentActivityType.STREAM_PREVIEW }
        val activities = if (existing >= 0) {
            run.activities.toMutableList().also { list ->
                list[existing] = list[existing].copy(streamSegments = segments)
            }
        } else {
            // Pin the live streaming item at the top of the Timeline
            listOf(
                AgentActivityItem(
                    type = AgentActivityType.STREAM_PREVIEW,
                    title = "",
...
// ---------------------------------------------------------------------------
// Live streaming parsing — classifies the cumulative text of the AI Studio
// response-in-progress into explanatory prose vs. code, so raw source is
// never rendered or retained. Fenced ```blocks are the primary signal; when
// AI Studio streams code without fences (the common case for this bridge,
// since it reads the rendered DOM rather than raw markdown) a line-level
// heuristic fallback is used instead. Pure functions only — no UI state is
// touched here, only the metadata objects returned to the caller.
// ---------------------------------------------------------------------------

private val FILENAME_LINE_REGEX =
    Regex("^(?:\\d+[.)]\\s*)?(?:code\\s*:\\s*)?([a-zA-Z0-9_][a-zA-Z0-9_\\-./]*\\.[a-zA-Z]{1,10})\\s*:?$", RegexOption.IGNORE_CASE)
private val FILENAME_TOKEN_REGEX = Regex("^[a-zA-Z0-9_][a-zA-Z0-9_\\-./]*\\.[a-zA-Z0-9]{1,10}$")
private val COMMENT_FILENAME_REGEX = Regex("^(?://|#)\\s*([a-zA-Z0-9_][a-zA-Z0-9_\\-./]*\\.[a-zA-Z0-9]{1,10})\\s*$")
private val HTML_COMMENT_FILENAME_REGEX = Regex("<!--\\s*([a-zA-Z0-9_][a-zA-Z0-9_\\-./]*\\.[a-zA-Z0-9]{1,10})\\s*-->")

/** Line-start patterns that are strong signals of raw source, fenced or not. */
private val CODE_LINE_REGEXES = listOf(
    Regex("^<!DOCTYPE", RegexOption.IGNORE_CASE),
    Regex("^</?[a-zA-Z][\\w-]*(\\s[^<>]*)?/?>$"), // a whole HTML tag on its own line
    Regex("^</?(script|style)\\b", RegexOption.IGNORE_CASE),
    Regex("^[.#]?[a-zA-Z][\\w-]*(\\s*,\\s*[.#]?[a-zA-Z][\\w-]*)*\\s*\\{$"), // CSS selector {
    Regex("^[a-zA-Z-]+\\s*:\\s*[^;{}]+;$"), // CSS property: value;
    Regex("^(function\\s|const\\s|let\\s|var\\s|import\\s|export\\s|class\\s|def\\s|fun\\s|@[A-Z][A-Za-z]*|public\\s|private\\s|package\\s)"),
    Regex("^[}\\])];?$") // a lone closing brace / bracket / paren
)

private fun isLikelyCodeLine(rawLine: String): Boolean {
    val line = rawLine.trim()
    if (line.isEmpty()) return false
    return CODE_LINE_REGEXES.any { it.containsMatchIn(line) }
}

private fun isStrongCodeOpener(rawLine: String): Boolean {
    val line = rawLine.trim()
    return line.startsWith("<!DOCTYPE", ignoreCase = true) ||
        line.startsWith("<html", ignoreCase = true) ||
        line.startsWith("<style", ignoreCase = true) ||
        line.startsWith("<script", ignoreCase = true)
}
...
/** Best-effort filename for an unfenced code run, from a heading line above it or a comment inside it. */
private fun resolveFileNameForRun(runLines: List<String>, precedingLine: String?): String? {
    precedingLine?.trim()?.takeIf { it.isNotEmpty() }?.let { pline ->
        FILENAME_LINE_REGEX.find(pline)?.groupValues?.get(1)?.let { return it }
    }
    val firstLine = runLines.firstOrNull()?.trim().orEmpty()
    HTML_COMMENT_FILENAME_REGEX.find(firstLine)?.let { return it.groupValues[1] }
    COMMENT_FILENAME_REGEX.find(firstLine)?.let { return it.groupValues[1] }
    if (firstLine.startsWith("<!DOCTYPE", ignoreCase = true) || firstLine.startsWith("<html", ignoreCase = true)) {
        return "index.html"
    }
    return null
}

private fun resolveLanguageForRun(runLines: List<String>, fileName: String?): String {
    fileName?.substringAfterLast('.', "")?.takeIf { it.isNotBlank() }?.let { return languageForExtension(it) }
    val sample = runLines.joinToString(" ").take(400)
    return when {
        sample.contains("<!DOCTYPE", true) || sample.contains("<html", true) -> "HTML"
        Regex("[.#]?[a-zA-Z][\\w-]*\\s*\\{").containsMatchIn(sample) -> "CSS"
        sample.contains("function ") || sample.contains("const ") || sample.contains("import ") -> "JS"
        sample.contains("def ") -> "PY"
        else -> "CODE"
    }
}

private fun resolveFileNameFromFenceInfo(info: String): String? =
    info.split(Regex("\\s+")).firstOrNull { it.contains('.') && FILENAME_TOKEN_REGEX.matches(it) }

private fun resolveLanguageFromFenceInfo(info: String, fileName: String?): String {
    fileName?.substringAfterLast('.', "")?.takeIf { it.isNotBlank() }?.let { return languageForExtension(it) }
    val lang = info.split(Regex("\\s+")).firstOrNull()?.lowercase().orEmpty()
    return when (lang) {
        "html" -> "HTML"
        "css" -> "CSS"
        "js", "javascript" -> "JS"
        ...
        "tsx" -> "TSX"
        "svg" -> "SVG"
        else -> if (lang.isNotBlank()) lang.uppercase() else "CODE"
    }
}

/**
 * Splits a chunk of text that is known to sit outside any ``` fence into prose and (heuristically
 * detected) code runs, appending resulting segments to [into]. [nextOrdinal] hands out stable,
 * increasing card keys so a given code block keeps the same key across repeated calls on growing
 * cumulative text, which lets the UI animate a card in once rather than replaying its entrance.
 */
private fun appendHeuristicSegments(
    text: String,
    into: MutableList<TimelineStreamSegment>,
    nextOrdinal: () -> Int
) {
    if (text.isBlank()) return
    val lines = text.split("\n")
    val isCode = BooleanArray(lines.size) { i -> isLikelyCodeLine(lines[i]) }

    // Demote isolated single-line "code" runs unless the line is a strong, unambiguous opener —
    // avoids flickering a file card into existence over one stray line inside a prose paragraph.
    var i = 0
    while (i < lines.size) {
        if (isCode[i]) {
            var j = i
            while (j + 1 < lines.size && isCode[j + 1]) j++
            if (j == i && !isStrongCodeOpener(lines[i])) {
                isCode[i] = false
            }
            i = j + 1
        } else {
    ...
    while (i < lines.size) {
        var j = i
        while (j + 1 < lines.size && isCode[j] == isCode[j + 1]) j++
        val runLines = lines.subList(i, j + 1)
        if (isCode[i]) {
            val fileName = resolveFileNameForRun(runLines, precedingLine = lines.getOrNull(i - 1))
            val language = resolveLanguageForRun(runLines, fileName)
            into.add(
                TimelineStreamSegment.FileCard(
                    cardKey = "code-${nextOrdinal()}",
                    fileName = fileName,
                    language = language,
                    lineCount = runLines.size,
                    state = FileCardState.READY // corrected in finalizeStreamCardStates()
                )
            )
        } else {
            val proseText = runLines.joinToString("\n").trim()
            if (proseText.isNotEmpty()) into.add(TimelineStreamSegment.Prose(proseText))
        }
        i = j + 1
    }
}

/** Fixes up whichever [TimelineStreamSegment.FileCard] is the actively-growing one (if any). */
private fun finalizeStreamCardStates(
    segments: List<TimelineStreamSegment>,
    existingPaths: Set<String>
): List<TimelineStreamSegment> {
    val lastFileCardIndex = segments.indexOfLast { it is TimelineStreamSegment.FileCard }
    val lastSegmentIsActiveCard = segments.lastOrNull() is TimelineStreamSegment.FileCard
    return segments.mapIndexed { index, segment ->
        if (segment !is TimelineStreamSegment.FileCard) return@mapIndexed segment
        val isActive = lastSegmentIsActiveCard && index == lastFileCardIndex
        val state = when {
            !isActive -> FileCardState.READY
            segment.lineCount < 2 -> FileCardState.CREATING
            segment.fileName != null && existingPaths.contains(segment.fileName) -> FileCardState.UPDATING
            else -> FileCardState.WRITING
        }
...
/**
 * Parses the cumulative, currently-visible text of the in-progress AI Studio reply into ordered
 * [TimelineStreamSegment]s: explanatory prose rendered as-is, and code — fenced or not — reduced
 * to file-card metadata only. [existingPaths] (current project file paths/names) is used purely
 * to decide whether an actively-streaming file should read as "Updating" vs "Writing".
 */
private fun buildTimelineStreamSegments(raw: String, existingPaths: Set<String>): List<TimelineStreamSegment> {
    if (raw.isBlank()) return emptyList()
    val segments = mutableListOf<TimelineStreamSegment>()
    var ordinal = 0
    fun nextOrdinal() = ordinal++

    val fenceParts = raw.split("```")
    if (fenceParts.size > 1) {
        fenceParts.forEachIndexed { idx, part ->
            if (idx % 2 == 0) {
                appendHeuristicSegments(part, segments, ::nextOrdinal)
            } else {
                val infoLine = part.lineSequence().firstOrNull()?.trim().orEmpty()
                val body = part.substringAfter('\n', missingDelimiterValue = "")
                val lineCount = if (body.isBlank()) 0 else body.split("\n").size
                val fileName = resolveFileNameFromFenceInfo(infoLine)
                val language = resolveLanguageFromFenceInfo(infoLine, fileName)
                segments.add(
                    TimelineStreamSegment.FileCard(
                        cardKey = "code-${nextOrdinal()}",
                        fileName = fileName,
                        language = language,
                        lineCount = lineCount,
                        state = FileCardState.READY // corrected below; unterminated trailing fence stays active
                    )
                )
            }
        }
    } else {
        appendHeuristicSegments(raw, segments, ::nextOrdinal)
    }

    return finalizeStreamCardStates(segments, existingPaths)
}
