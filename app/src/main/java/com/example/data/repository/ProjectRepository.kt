...
class ProjectRepository(private val database: CodePilotDatabase) {
    ...
    suspend fun getProjectByChatId(chatId: String): ProjectEntity? =
        if (chatId.isBlank()) null else projectDao.getProjectByChatId(chatId)
    ...
    suspend fun createProject(name: String, source: String = "ai_studio", chatId: String = ""): ProjectEntity {
    ...
    /** Renames a project and marks the name as user/AI-resolved so it is never auto-renamed again. */
    suspend fun renameProject(projectId: String, newName: String): ProjectEntity? {
        val clean = newName.trim()
        if (clean.isBlank()) return null
        projectDao.renameProject(projectId, clean, System.currentTimeMillis())
